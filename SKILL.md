...
# Requirement Clarifier（需求澄清器）v2.0

使用者是**开发者本人**，直接对接一个不懂 PRD、不懂技术的业务方。业务给的需求口语化、跳跃、充满隐含假设。本 skill 的目标是把开发和业务间痛苦的来回压到最少，并让每次成果沉淀下来。
...
3. **AI 不替人拍板。** 值不值得做、选哪个方案、优先级，是业务和开发的决策；skill 只提供信息、选项、代价。
4. **结论必贴溯源标签，原样【】全角格式**（丢括号 = 丢标签，机检按字面 grep，时间再紧不许简写）：
   - **【业务确认】** 明确决定，带日期。
   - **【开发拟定】** 业务只给方向（"按角色过滤"），细则由开发落成具体默认规则，须放确认单送业务过目、无异议生效。
...
不存在则首次使用时创建（创建前告知用户），模板见 `templates/`。**每次被触发，第一件事读 context.md 和相关 spec**——已确认过的绝不重复问。**每轮结束主动写回**新黑话、新回答、新约束、新风险。沉淀不是可选项。
...
问题清单要求：只列业务才能回答的（技术选型开发自己定）；一律选择题化（业务擅长选 A/B，不擅长从零描述）；已有答案的不问；按阻塞程度排序；**每题标建议回答人**——知情人常散在多拨人手里（提需求业务、财务、外围系统操作者等），**警惕"替答"**：别让提需求的人替财务或其他团队拍板。确认单额外要求：①开头附"已确认事项核对表"，把口头共识变成有落款的凭证；②每题 ☐ 选项 + 作答区；③"建议由 XX 回答，不归您管请转交"；④给"我不清楚"台阶，顺势索要真正知情人。
...
2. **与已确认决定做冲突检测。** 每条新答案对照 context.md 与 spec 既有决定——撞旧决定是高频事故，当场揪出让业务二选一，绝不无声合并。
...
**业务沉默协议：** 确认单注明回填时限（默认 3 工作日）。超时 → 提醒一次并抄送拍板人；再超时 → 【开发拟定】转"**超时临时生效**"，spec 与 changes.md 记时间戳与"业务未回复，临时生效，风险归需求方"，保持临时标记直至补落款——**超时生效 ≠ 业务确认，标签永不因超时转正**；阻塞级不适用降级，只能推迟开发或向拍板人升级。

**落款检查：** 回收确认单核对填写人/日期；回执（确认单/聊天/邮件）归档进 `raw/`，spec 确认记录引用回执行号。缺落款提醒补。注意 provenance：业务"说过"≠"决定了"，随口畅想按【假设】入账。

**验收后合并，产出两份：**

- **① 开发规格** → `specs/<功能名>.md`（模板 `spec-template.md`）：只写实现层——功能说明、输入/输出、处理逻辑、数据结构、边界、异常、验收标准，每条关键决定带溯源标签与日期。不写 ROI、灰度、里程碑。
- **② 业务确认单** → 纯白话零术语（模板 `confirmation-template.md`）："你要的是……当……时系统会……这次**不包含**……"。【开发拟定】默认规则单独列出请业务重点过目。业务签认的是这份，是日后"我没说过"的依据。
...
不需用户先报障。方法必读 `references/chain-audit-checklist.md`：三份盘点（实体状态/操作入口/**不变量，钱优先**）→ 组合矩阵（按优先级收敛，一次审一族）→ 七种缺陷模式逐条对照 → 分流：守卫加固类直接给修复项；**规则未定义类选择题化进业务问题清单，走模式 A 阶段三管道**。审计沉淀进 specs/ 与 context.md，越审越快。
...
- **阻塞级岔口必须配跨分支数值示例**：同一组输入，每个候选选项各算一遍摆对照表，并标注该示例能/不能区分哪些选项（区分不了就换例子）。只算一个选项 = 暗中替业务拍板；只画岔口不算数 = 让人凭抽象拍板。表头注明"演示数字，非任何选项的背书"。
